...
```
node "${CLAUDE_PLUGIN_ROOT}/scripts/ledger.js" next-id L
```
...
- If it should change how features get planned, mention it in `/ledgerline:start-feature` (that skill reads `LESSONS.md` before planning — make sure your rule is findable there).
- If three lessons point the same direction, that is a signal about the process, not the code. Say so during `/ledgerline:milestone-review`.
