...
- The feature maps to no requirement. That is scope creep → `/ledgerline:roadmap`, and that skill stops for agreement before anything gets built. Do not route through it and carry on in the same turn.
...
- A non-obvious choice gets made → `/ledgerline:decide`, in the moment.
- Something gets deliberately skipped → `/ledgerline:followup`, with a trigger.
