---
name: kickoff
description: Start a new project under Ledgerline tracking, from an existing PRD document or from a rough idea. Produces the PRD, initial architecture, and a dependency-ordered roadmap of milestones and features, then hands the first feature to Superpowers. Use when starting an application from scratch, when handed a PRD or spec to build, or when adopting Ledgerline in a repo that has no docs/project/ ledger.
...
```
node "${CLAUDE_PLUGIN_ROOT}/scripts/ledger.js" paths
```

If a ledger already exists, stop. Say so, and offer `/ledgerline:resume` or
`/ledgerline:roadmap` instead. Kickoff runs once per project.
...
```
node "${CLAUDE_PLUGIN_ROOT}/scripts/ledger.js" init --name "<project name>"
```
...
- Run `ledger.js state` to generate `STATE.md`, then `ledger.js check`, and fix everything it reports.
...
before code exists than after. When they agree, `/ledgerline:start-feature`.
