#!/usr/bin/env node
/**
 * ledger.js — the Ledgerline engine.
 *
 * Dependency-free, cross-platform (Windows/macOS/Linux) CLI used by Ledgerline
...
function check(m, opts) {
  ...
  if (!m.hasLedger) {
    return [{ level: 'error', msg: 'No ledger found (docs/project/ROADMAP.md missing).', fix: 'Run /ledgerline:kickoff to create one.' }];
  }

  for (const f of LEDGER_FILES) {
    if (m.files[f] === null) W(`Ledger file ${f} is missing.`, 'Re-run `ledger.js init` to restore it from template.');
  }
  ...
  for (const f of m.features) {
    ...
    if (needsDossier && !f.dossierExists) {
      E(`${f.id} is ${f.status} but has no dossier file.`, 'Run /ledgerline:start-feature, or create the dossier from templates/FEATURE.md.');
    }
    if (f.dossierExists) {
      if (f.dossierStatus && f.dossierStatus !== f.status) {
        E(`${f.id}: roadmap says "${f.status}", dossier says "${f.dossierStatus}".`, 'Reconcile with /ledgerline:resume; the roadmap wins unless git says otherwise.');
      }
  ...
  if (!m.features.length) W('The roadmap has no features.', 'Run /ledgerline:kickoff or /ledgerline:roadmap to populate it.');
  ...
  if (!forState) {
    ...
    if (onDisk === null) {
      W('STATE.md has not been generated yet.', 'Run `ledger.js state`.');
      ...
      W('STATE.md is stale or was hand-edited (it is a generated file).',
        'Run `ledger.js state` to regenerate. If you meant to change a fact, change the file that owns it.');
    }
  }
...
function generateState(m) {
  ...
  line('     Regenerate: node <plugin>/scripts/ledger.js state');
  line('     Hand edits are destroyed on the next regeneration, and `ledger.js check` reports them. -->');
  line('');
  ...
  if (needs.length) {
    line('## Needs a person');
    line('');
    for (const n of cap(needs, 5, 'PRD.md / FOLLOWUPS.md / `ledger.js check`')) line(n.__more ? n.__more : `- ${n}`);
    line('');
  }
  ...
  if (!ready.length) {
    line(!m.features.length
      ? 'The roadmap has no features yet — run /ledgerline:kickoff.'
      : live === done
  ...
  line(errs.length
    ? `Check: ${errs.length} error${errs.length === 1 ? '' : 's'}, ${warns.length} warning${warns.length === 1 ? '' : 's'} — run \`ledger.js check\`.`
...
function brief(m) {
  ...
  out.push(`Ledgerline ledger at docs/project/ — project "${m.name}" (${done}/${total} features done).`);
  if (ms) {
  ...
  out.push('Read the skill ledgerline:using-ledgerline before editing any ledger file. Run /ledgerline:resume before starting new work in a fresh session.');
  return out.join('\n');
}

function statusMd(m) {
  if (!m.hasLedger) {
    return `No Ledgerline ledger found under ${m.root}.\nRun /ledgerline:kickoff to create one from a PRD or an idea.`;
  }
...
function init(loc, opts) {
  ...
  if (missing.length) {
    ...
    fs.writeFileSync(gaPath, existing + prefix +
      (existing ? '\n' : '') + '# Ledgerline: append-only ledger files\n' + missing.join('\n') + '\n');
...
function main(argv) {
  ...
  // so `ledger.js --cwd /x status` and `ledger.js status --cwd /x` behave the same.
  ...
  switch (cmd) {
    ...
    case 'next-id': {
      ...
      if (!id) { process.stderr.write('usage: ledger.js next-id <F|D|U|L|R|M>\n'); return 2; }
    ...
    default:
      process.stderr.write(
        'ledger.js — Ledgerline ledger engine\n\n' +
        'usage: node ledger.js <init|state|status|check|next-id|today|paths> [options]\n' +
        '  init   [--name "X"] [--force]\n' +
        '  state  [--stdout]\n' +
        '  status [--brief|--json]\n' +
        '  check\n' +
...
if (require.main === module) {
  try {
    ...
    process.stderr.write('ledger.js failed: ' + (err && err.stack || err) + '\n');
    process.exitCode = 3;
  }
}
