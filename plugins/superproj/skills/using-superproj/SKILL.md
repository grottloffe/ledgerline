...
| "`sp.js check` passes, so the ledger is fine" | `check` validates consistency, not authority. A ledger edited to match unapproved work passes cleanly. |
