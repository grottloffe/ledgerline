...
```
node "${CLAUDE_PLUGIN_ROOT}/scripts/sp.js" next-id U
```
...
If it needs its own feature row, put it on the roadmap via `/superproj:roadmap`
...
- Follow-ups whose trigger has fired are surfaced by `/superproj:status`, but only a person reading the triggers can tell. Do that reading during `/superproj:milestone-review`.
