...
when_to_use: Trigger on "where were we", "let's continue", "pick up where we left off", "what was I doing", and at the start of a session in a repo with a SuperProj ledger.
---
...
| Signal | Likely truth |
|---|---|
| Feature `in-progress`, its branch merged, tests pass | It landed and was never closed → `/superproj:finish-feature` |
